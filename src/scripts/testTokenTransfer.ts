...
import { initializePlatformWallets, sendTokenToUser } from '../services/platformWallet';
...
const CHAIN = 'arbitrum' as Chain;
const TOKEN = 'USDC' as TokenSymbol;
...
async function testTokenTransfer() {
  try {
    ...
    const platformBalance = await getTokenBalanceOnChain(platformWallets.main.address, CHAIN, TOKEN);
    ...
    const userBalance = await getTokenBalanceOnChain(USER_WALLET_ADDRESS, CHAIN, TOKEN);
    ...
    const newUserBalance = await getTokenBalanceOnChain(USER_WALLET_ADDRESS, CHAIN, TOKEN);
