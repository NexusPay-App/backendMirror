...
exports.getTransactionById = exports.getTransactionHistory = void 0;
...
const getTransactionHistory = async (req, res) => {
    try {
        ...
        if (!req.user) {
            return res.status(401).json((0, utils_1.standardResponse)(false, 'Authentication required', null, { code: 'AUTH_REQUIRED', message: 'You must be logged in to view your transaction history' }));
        }
        ...
        const totalTransactions = await escrowModel_1.Escrow.countDocuments({ userId });
        // Fetch transactions for user with pagination
        const transactions = await escrowModel_1.Escrow.find({ userId })
        ...
        return res.json((0, utils_1.standardResponse)(true, 'Transaction history retrieved successfully', {
        ...
        return (0, utils_1.handleError)(error, res, 'Failed to retrieve transaction history');
    }
};
...
const getTransactionById = async (req, res) => {
    try {
        // Make sure user is authenticated
        if (!req.user) {
            return res.status(401).json((0, utils_1.standardResponse)(false, 'Authentication required', null, { code: 'AUTH_REQUIRED', message: 'You must be logged in to view transaction details' }));
        }
        const userId = req.user._id;
        const { id } = req.params;
        ...
        if (!transaction) {
            return res.status(404).json((0, utils_1.standardResponse)(false, 'Transaction not found', null, { code: 'TRANSACTION_NOT_FOUND', message: 'Transaction with the provided ID does not exist or does not belong to you' }));
        }
        ...
        return res.json((0, utils_1.standardResponse)(true, 'Transaction details retrieved successfully', { transaction: formattedTransaction }));
    }
    catch (error) {
        return (0, utils_1.handleError)(error, res, 'Failed to retrieve transaction details');
    }
};
