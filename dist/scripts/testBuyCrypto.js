...
async function testBuyCrypto() {
    try {
        ...
        const user = await models_1.User.findOne({ phoneNumber: PHONE_NUMBER });
        if (!user) {
        ...
        const celoStatus = await (0, platformWallet_1.getPlatformWalletStatus)('celo');
        const arbitrumStatus = await (0, platformWallet_1.getPlatformWalletStatus)('arbitrum');
        ...
        const response = await axios_1.default.post(`${API_URL}/mpesa/buy-crypto`, {
            cryptoAmount: amount,
            phone: PHONE_NUMBER,
            chain,
            tokenType
        ...
        if (response.data?.data?.transactionId) {
            ...
            try {
                ...
                const statusResponse = await axios_1.default.get(`${API_URL}/transactions/status/${transactionId}`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
