 ...
 */
async function getConversionRateWithCaching(tokenType = 'USDC') {
    ...
    try {
        ...
        const acquired = await redis.set(RATE_LOCK_KEY, lockId, 'EX', LOCK_DURATION, 'NX');
 ...
 * @returns The current conversion rate
 */
async function fetchCryptoToKESPrice(tokenType = 'USDC') {
    try {
 ...
 * @param tokenType The type of token (USDC, USDT, BTC, ETH)
 */
async function forceRefreshConversionRate(tokenType = 'USDC') {
    try {
        // Normalize token type
        const token = tokenType.toUpperCase();
        // Use default rate if token not supported
